/* ============================================================
   ...
   Routes (API Gateway HTTP API, $default):
     ...
     GET  /api/secrets/{id}/meta    -> exists + viewsLeft (no burn)
...
function cors(origin) {
  const ok = /^https:\/\/([a-z0-9-]+\.)*jakvab\.se$/.test(origin || "");
  return {
...
export const handler = async (event) => {
  ...
  // GET /api/secrets/{id}/meta — no burn
  ...
  if (m && method === "GET") {
    ...
    return reply(200, { exists: true, viewsLeft: item.views });
  }
